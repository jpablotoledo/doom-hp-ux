...
static void PacketSend (void)
{
	...
	sw.cmds[c].newflags = netbuffer->cmds[c].newflags;
	sw.cmds[c].vaccel = netbuffer->cmds[c].vaccel;
	sw.cmds[c].misc1 = netbuffer->cmds[c].misc1;
	sw.cmds[c].misc2 = netbuffer->cmds[c].misc2;
#endif
    }
...
#ifdef FULL_NEW_FEATURES
	netbuffer->cmds[c].newflags = sw.cmds[c].newflags;
	netbuffer->cmds[c].vaccel = sw.cmds[c].vaccel;
	netbuffer->cmds[c].misc1 = sw.cmds[c].misc1;
	netbuffer->cmds[c].misc2 = sw.cmds[c].misc2;
#endif
    }
}
...
{
#ifdef __UNIXLIB_TYPES_H
  if (insocket!=-1)   { close(insocket); insocket=-1; }
  if (sendsocket!=-1) { close(sendsocket); sendsocket=-1; }
#else
  if (insocket!=-1)   { socketclose(insocket);   insocket=-1; }
  if (sendsocket!=-1) { socketclose(sendsocket); sendsocket=-1; }
#endif
}
